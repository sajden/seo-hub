...
export async function semanticCompare(topic, existingArticles) {
  const articleSummaries = existingArticles
    .map((article) => {
      ...
      return `- ${article.title} (tags: ${article.tags?.join(', ') || 'none'}${status}${topicHint}${keywordHint}${angleHint}${descriptionHint})`;
    })
    .join('\n');
...
export async function findContentGaps(existingArticles, existingDrafts = [], siteConfig) {
  const articleSummaries = existingArticles.map((article) => `- ${article.title}`).join('\n');
