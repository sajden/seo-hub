...
function applyRuntimeOverrides(config) {
  const from = process.env.SEO_HUB_REPO_PATH_REWRITE_FROM?.trim();
  const to = process.env.SEO_HUB_REPO_PATH_REWRITE_TO?.trim();
  ...
  return {
    ...
    sites: config.sites.map((site) => ({
      ...site,
      targetRepo: typeof site.targetRepo === 'string' && site.targetRepo.startsWith(from)
        ? `${to}${site.targetRepo.slice(from.length)}`
        : site.targetRepo,
    })),
  };
}
...
export function loadConfig() {
  ...
  if (!cachedConfig) {
    try {
      ...
      cachedConfig = applyRuntimeOverrides(JSON.parse(raw));
